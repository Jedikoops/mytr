...
function getWordAtLocation(aCanvasX, aCanvasY) { // mouse click
...
function drawCanvas() {
  ...
  for (let i = 0; i < words.length; i++) { //note i declared as var
    ...
    words[i].myBox.width = context.measureText(words[i].word).width
    ...
    if(data.isSelected == true){
      context.strokeRect(data.x - 2, data.y - data.myBox.height + 2, data.myBox.width + 4, data.myBox.height + 4);
    }
